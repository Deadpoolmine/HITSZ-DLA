...
`define TV_IN_out_r "../tv/cdatafile/c.matrixmul.autotvin_out_r.dat"
...
parameter out_r_data_in_addr = 32;
...
assign TRAN_AXILiteS_write_data_finish = 1 & A_write_data_finish & B_write_data_finish & out_r_write_data_finish;
            ...
            if (write_B_run_flag === 1 && get_ack === 1) begin
                ...
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (B_c_bitwidth < 32) begin
                        B_data_tmp_reg = reg_B;
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < B_c_bitwidth) begin
                                B_data_tmp_reg[j] = reg_B[i*32 + j];
                            end
                            else begin
                                B_data_tmp_reg[j] = 0;
                            end
